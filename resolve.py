...
api = twitter.Api(consumer_key=oauth['CONSUMER_KEY'], consumer_secret=oauth['CONSUMER_SECRET'],
                  access_token_key=oauth['ACCESS_TOKEN_KEY'], access_token_secret=oauth['ACCESS_TOKEN_SECRET'],
...
for user in users:
    while True:
        user = user.strip()
        try:
            blocked = api.CreateBlock(screen_name=user, include_entities=False, skip_status=True)
            print('%s->%s BLOCKED' % (user, blocked.id))
            break
        except twitter.error.TwitterError as e:
            print("error: twitter: %s" % (e), file=sys.stderr)
            break
        except OSError:
            # probably network down
            print("error: general: %s" % (e), file=sys.stderr)
            pass
